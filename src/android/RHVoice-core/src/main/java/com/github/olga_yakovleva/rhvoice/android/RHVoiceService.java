...
public final class RHVoiceService extends TextToSpeechService implements LifecycleOwner {
    ...
    private Context storageContext = RHVoiceApp.getStorageContext();
    ...
    private void initialize() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
            storageContext.moveSharedPreferencesFrom(this, this.getPackageName() + "_settings");
    ...
    private class VoiceInstaller implements Runnable, Observer<PackageDirectory> {
        ...
        public void run() {
            ...
            dataManager.autoInstallVoice(storageContext, languageCode, countryCode);
        }

        public void onChanged(PackageDirectory dir) {
            Repository.get().getPackageDirectoryLiveData().removeObserver(this);
            new DataManager(dir).autoInstallVoice(storageContext, languageCode, countryCode);
        }
    }
